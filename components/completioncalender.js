...
class MaterialCalender {
    ...
    handleday(day) {
        const timein = new CompletionDate();
        if (day < 10) {
            day = `0${day}`
        }
    ...
    showicon() {
        ...
        if (this.state.completioncalender) {
            return (
                <TouchableOpacity onPress={() => { this.setState({ completioncalender: false }) }}>
            ...
            return (
                <TouchableOpacity onPress={() => { this.setState({ completioncalender: true }) }}>
    ...
    activecell(num) {
        if (Number(num) === Number(this.state.completiondateday)) {
